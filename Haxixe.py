...
USAGE = """
	  ...
	  ┏┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┷┓
          ┫ 2.7.x <= Compativel com Python  <= 3.x.x ┣
	  ┗┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┳┛
"""
...
def start_process():
    ...
    while True:
        ...
        if firstrun and len(argv) == 2:
            ...
            input_hash = get_input("Insira o Hash : ")
            # Trim the hash entered and remove the unwanted spaces
            input_hash = input_hash.strip()

            # Exit from loop.
            if input_hash in ("quit", "exit", "q", "end"):
                print("Ate Breve! Bola+1 ;) .")
                break

        if len(input_hash) < 1:
            print("\nPor favor, digite o hash. Nenhum hash de entrada foi encontrado.")
        else:
            ...
            if len(results) == 0:
                print("\n\n:( Desculpe, não conseguimos identificar o tipo de hash.")
            elif len(results) > 2:

                # Show the results with most and less probable hash algorithms
                print("\nAlgoritmos de Hash mais Prováveis ​​Encontrados:\n")
                print("[✔] " + results[0])
                print("[✔] " + results[1])
                print("\nOutros Algoritmos de Hash Possíveis encontrados:\n")
...
def main():
    """
    Imprima o TITLE e USAGE e, em seguida, inicie o loop principal.
    """
    print(TITLE)
    print(USAGE)
    try:
        ...
        print("Encerramento solicitado ... saindo")
    except EOFError:
        print("\nSistema encerrado durante a entrada do usuário.")
